...
from data import read_steerings, read_image_stamps, read_images, read_images_augment, data_generator, normalize_input, exact_output, preprocess_input_InceptionV3
from model import build_cnn, build_InceptionV3
...
def main():
    ...
    parser.add_argument('--model', type=str, help='model to evaluate, current list: {cnn, InceptionV3}')
    parser.add_argument('--resized-image-height', type=int, default=128, help='image resizing')
    parser.add_argument('--resized-image-width', type=int,default=128, help='image resizing')
    parser.add_argument('--epochs', type=int, help='# of training epoch')
    ...
    # Use data.py to generate training data
    START_TIMESTAMP_TRAIN = 14794242158
    END_TIMESTAMP_TRAIN = 14794265723 - 1600
    START_TIMESTAMP_VAL = 14794265723 - 1600 + 1
    END_TIMESTAMP_VAL = 14794265723
    ...
    if not weights_path:
        train_generator = data_generator(steering_log=steering_log,
                            ...
                            timestamp_start=START_TIMESTAMP_TRAIN,
                            timestamp_end=END_TIMESTAMP_TRAIN,
                            shuffle=True,
        ...
        val_generator = data_generator(steering_log=steering_log,
                            ...
                            image_size=image_size,
                            # Round 2 Complete Dataset
                            timestamp_start=START_TIMESTAMP_VAL,
                            timestamp_end=END_TIMESTAMP_VAL,
                            shuffle=False,
                            preprocess_input=input_processor,
                            preprocess_output=output_processor)
